        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    var buttons = '<a href="/erp/caso/update/' + row.id + '/" class="btn btn-warning btn-xs btn-flat"><i class="fas fa-edit"></i></a> ';
                    buttons += '<a href="/erp/caso/delete/' + row.id + '/" type="button" class="btn btn-danger btn-xs btn-flat"><i class="fas fa-trash-alt"></i></a> ';
...
$(function () {
    ...
    $('#data tbody').on('click', 'a[rel="edit"]', function () {
        ...
        modal_title.find("span").html("Detalles de Caso");
        ...
        var data = tblClient.row(tr.row).data();
            ...
            $('textarea[name="resultado"]').val(data.resultado);
            // $('date[name="fecha_realizacion"]').val(data.fecha_realizacion);
            // $('input[name="completado"]').val(data.completado);
        $('#myModalCaso').modal('show');
    });



    $('#myModalCaso').on('shown.bs.modal', function () {
        // $('form')[0].reset();
    });
